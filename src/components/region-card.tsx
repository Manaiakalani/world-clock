...
export function RegionCard({ region, now, onClick, isActive }: RegionCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative w-full overflow-hidden rounded-xl border p-3 text-left
    ...
    >
      <div className="relative z-10 flex items-center gap-3">
        ...
        <div className="text-right">
          <div className={`text-sm font-mono font-bold tabular-nums ${dark ? "text-white" : "text-gray-900"}`}>
            {time}
          </div>
          <div className={`text-xs ${offset === 0 ? (dark ? "text-white/50" : "text-gray-400") : dark ? "text-white/60" : "text-gray-600"}`}>
            {offsetStr}
          </div>
        </div>
      </div>
